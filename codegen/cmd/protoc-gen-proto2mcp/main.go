...
func main() {
	...
	protogen.Options{}.Run(func(gen *protogen.Plugin) error {
		...
		for _, fileIR := range fileIRs {
			...
			if len(infos) == 0 && len(fileIR.Prompts) == 0 {
				continue
			}
			...
			if len(infos) > 0 {
				...
				jFile = jen.NewFilePathName(string(protoFile.GoImportPath), string(protoFile.GoPackageName))
				jFile.PackageComment("Code generated by protoc-gen-proto2mcp. DO NOT EDIT.")
			}

			if len(fileIR.Prompts) > 0 {
				// Derive a base name for the register function from the proto file name.
				// e.g., "patient.proto" -> "Patient"
				baseName := extractBaseName(fileIR.FileName)
				emit.GeneratePrompts(jFile, fileIR.Prompts, "Register"+baseName+"Prompts")
			}
...
func extractBaseName(path string) string {
	...
	// Title case for function name, remove underscores
